...
var (
	...
	DefaultCourseGeneratorAgent adk.Agent
	DefaultCourseGeneratorRunner *adk.Runner
)
...
func InitPlanRunner(ctx context.Context){
    ...
    DefaultPlanRunner = NewStreamRunner(ctx, DefaultPlanAgent)
}
...
func InitRecommendRunner(ctx context.Context){
    ...
    DefaultRecommendRunner = NewStreamRunner(ctx, DefaultRecommendAgent)
}
...
func InitWebSearchRunner(ctx context.Context){
    ...
    DefaultWebSearchRunner = NewStreamRunner(ctx, DefaultWebSearchAgent)
}

func InitCourseGeneratorRunner(ctx context.Context){
	// 合并多个工具
	var allTools []tool.BaseTool
	// allTools = append(allTools, config.ToolMap["jina"]...)    // jina工具
	// allTools = append(allTools, config.ToolMap["fetch"]...)  // fetch工具
	...
	name := "课程生成小助手"
	description := "一个帮助他人生成课程的助手"
	systemPrompt := "你是一个可以使用工具帮助他人生成课程的助手，在生成课程的时候先使用jina\fetch相关工具来搜索用户需要的内容并进行一定总结，然后根据用户的喜好来生成课程。"
	DefaultCourseGeneratorAgent = NewAgent(ctx, name, description, systemPrompt, allTools)
    DefaultCourseGeneratorRunner = NewTrunkRunner(ctx, DefaultCourseGeneratorAgent)
}